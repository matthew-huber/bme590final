...
@pytest.mark.parametrize("input_img, expected_img", [
    ...
    (np.array([[1, 2], [3, 4], [5, 6]]), np.array([[5, 6], [3, 4], [1, 2]])),
    (np.array([[5, 20], [3, 4], [8, 10]]),
     np.array([[8, 10], [3, 4], [5, 20]])),

])
def test_compress_multidimm_image(GUI, input_img, expected_img):
    assert(GUI.compress_multidimm_img(input_img) == expected_img)
...
def test_makeDatabaseFileNames(filenames, username, expected, GUI):
    assert GUI.makeDatabaseFileNames(filenames, username) == expected
...
def test_getfilenames_remove_full_path(filenames, expected, GUI):
    assert GUI.get_filenames_remove_full_path(filenames) == expected


@pytest.mark.parametrize("files, username, expected", [
    ("/file/with/slahes", "katelyn", "slaheskatelyn"),
...
@pytest.mark.parametrize("input_files, output_files", [
    ...
    (["TestImages/GS2D.png", "TestImages/GS2D.png"],
     ["TestImages/GS2D.png", "TestImages/GS2D.png"]),

    (["TestImages/GS2D.png", "TestImages/not_an_image.png",
      "TestImages/color_img.jpg"],
     ["TestImages/GS2D.png", "TestImages/color_img.jpg"])
    ])
